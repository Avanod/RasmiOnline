...
var chartElementIds = ['paymentLastDays',
                        'paymentLastMonths',
                        'otherItemPaymentLastDays',
                        'otherItemPaymentLastMonths',
                        'licenseItemPaymentLastDays',
                        'licenseItemPaymentLastMonths'];

$(document).ready(function () {
    let rand = randomColor.get(6);
    for (var i = 0; i < chartElementIds.length; i++) {
        let $canvas = $('#' + chartElementIds[i]);
        let chtData = {
            label: $canvas.data('label'),
            backgroundColor: rand[i].color,
            borderColor: rand[i].bColor,
            borderWidth: 1,
            data: Object.values(window[chartElementIds[i]])
        };
        if ($canvas.data('type') === 'bar') initBarChart($canvas, Object.keys(window[chartElementIds[i]]), chtData);
        else initLineChart($canvas, Object.keys(window[chartElementIds[i]]), chtData);
    }
